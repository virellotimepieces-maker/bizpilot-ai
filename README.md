...
Paid Email at `/app/email` can connect a Gmail account. Tokens are encrypted with `AUTH_SECRET` and stored only on the server. Suggested replies are generated by the email AI from the customer’s message, with Knowledge as business context (not pasted into the reply). **Send reply** asks for confirmation and never sends on its own.
...
- `/app/email` Connect Gmail inbox, AI suggested replies from the customer’s email (Knowledge is context), send only after confirm. Optional “Add email manually” fallback.
